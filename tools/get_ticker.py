...
from mcp.error import UserError
import json
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if not data:
                ...
                raise UserError(f"Upbit API returned empty data for symbol: {symbol}")
            return data[0]
        except httpx.HTTPStatusError as e:
            error_message = f"HTTP error occurred: {e.response.status_code} - {e.response.text}"
            print(f"ERROR: get_ticker {error_message}", flush=True)
            raise UserError(f"Upbit API request failed for {symbol}: {error_message}")
            ...
            print(f"ERROR: get_ticker {error_message}", flush=True)
            raise UserError(f"Upbit API returned invalid JSON for {symbol}: {error_message}")
            ...
            print(f"ERROR: get_ticker {error_message}", flush=True)
            raise UserError(f"Upbit API returned unexpected data structure for {symbol}: {error_message}")
        except Exception as e:
            error_message = f"An unexpected error occurred in get_ticker: {str(e)}"
            print(f"ERROR: get_ticker {error_message}", flush=True)
            raise UserError(f"An unexpected error occurred while fetching ticker for {symbol}: {error_message}")